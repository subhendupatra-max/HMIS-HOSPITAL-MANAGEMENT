...
class OpdController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->case_id != null || $request->opd_id != null || $request->patient_phone_no != null ||$request->patient_first_name != null ||$request->appointment_date != null || $request->patient_uhid != null){
        // dd($request->opd_id);
            $opd_registaion_list =  OpdDetails::where(function ($query)  use ($request,$trimmedString) {
                if (!auth()->user()->can('False Generation')) {
                    $query->where('opd_details.ins_by', 'ori');
                }
                if ($request->case_id != '') {
                    $query->where('opd_details.case_id', '=', $request->case_id);
                }
                if ($request->opd_id != '') {
                    $query->where('opd_details.id', '=', $request->opd_id);
                }
                ...
                ->whereHas('latest_opd_visit_details_for_patient',function ($query)  use ($request) {
            ...
            $opd_registaion_list =  OpdDetails::where(function ($query) {
                ...
                })->whereHas('latest_opd_visit_details_for_patient',function ($query)  use ($dateBefore7Days) {
        ...
        return view('OPD.opd-patient-list', compact('opd_registaion_list','request_data'));
    }
