                ...
                <div class="col-md-12">
                    <div class="row no-gutters">
                        <div class="col-md-8 border-right">
                            <form method="POST" action="{{ route('fetch-emg-patient-report') }}">
                                @csrf
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="form-group col-md-4 newaddappon">
                                            ...
                                            <select name="medico_legal_case" class="form-control select2-show-search"
                                                ...
                                                <option value="yes" {{@$all_search_data['medico_legal_case']=='yes'
                                                    ? 'selected' :'' }}>Yes</option>
                                                <option value="no" {{@$all_search_data['medico_legal_case']=='no'
                                                    ? 'selected' :'' }}>No</option>
                                            </select>
            ...
            <div class="">
                <div class="table-responsive">
                    <table class="table table-bordered text-nowrap" id="example">
                        ...
                        <tbody>
                            <?php $i = 0; ?>
                            @if (@$emg_patient_report[0]->id != null)
                            @foreach ($emg_patient_report as $value)
                            <?php $i++; ?>
                            <tr>
                                <td>{{ @$value->emg_details_data->emg_prefix }}{{ @$value->emg_details_data->id }}</td>
                                <td>{{ @$value->emg_details_data->all_patient_details->prefix }}
                                    {{ @$value->emg_details_data->all_patient_details->first_name }}
                                    {{ @$value->emg_details_data->all_patient_details->middle_name }}
                                    {{ @$value->emg_details_data->all_patient_details->last_name }}({{
                                    @$value->emg_details_data->all_patient_details->id }})</td>
                                <td> @if (@$value->emg_details_data->all_patient_details->year != 0)
                                    {{ @$value->emg_details_data->all_patient_details->year }}y
                                    @endif
                                    @if (@$value->emg_details_data->all_patient_details->month != 0)
                                    {{ @$value->emg_details_data->all_patient_details->month }}m
                                    @endif
                                    @if (@$value->emg_details_data->all_patient_details->day != 0)
                                    {{ @$value->emg_details_data->all_patient_details->day }}d
                                    @endif</td>
                                <td>{{ @$value->emg_details_data->all_patient_details->guardian_name }}</td>
                                <td>{{ @$value->emg_details_data->all_patient_details->phone }}</td>
                                <td>{{ @$value->patient_type }}</td>
                                <td>{{ @$value->emg_details_data->case_id }}</td>
                                <td>{{ date('d-m-Y h:i A',
                                    strtotime(@$value->appointment_date)) }}</td>
                                <td> {{ @$value->patient_department_details->department_name }}
                                </td>
