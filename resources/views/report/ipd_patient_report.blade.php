                ...
                <div class="col-md-12">
                    <div class="row no-gutters">
                        <div class="col-md-8 border-right">
                            <form method="POST" action="{{ route('fetch-ipd-patient-report') }}">
                                @csrf
                                <div class="col-md-12">
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-4 newaddappon">
                                            ...
                                            <select name="patient_type" onchange="getDetailsAccordingType(this.value)"
                                                ...
                                                @foreach (Config::get('static.patient_types') as $key =>
                                                $patient_type)
                                        ...
                                        <div class="form-group col-md-4 newaddappon">
                                            ...
                                            <select name="department" class="form-control select2-show-search"
                                                ...
                                                <option value="{{$department->id}}"
                                                    {{@$all_search_data['department']==$department->id ?
                                                    'selected':'' }}> {{$department->department_name}}
                                                </option>
                                                @endforeach
                                            </select>
                                            @error('department')
            ...
            <div class="">
                <div class="table-responsive">
                    <table class="table table-bordered text-nowrap" id="example">
                        ...
                        <tbody>
                            <?php $i = 0; ?>
                            @if (@$ipd_patient_report[0]->id != null)
                            @foreach ($ipd_patient_report as $value)
                            <?php $i++; ?>
                            <tr>
                                <td>{{ @$value->ipd_prefix }}{{ @$value->id }}</td>
                                <td>{{ @$value->all_patient_details->prefix }}
                                    {{ @$value->all_patient_details->first_name }}
                                    {{ @$value->all_patient_details->middle_name }}
                                    {{ @$value->all_patient_details->last_name }}({{
                                    @$value->all_patient_details->id }})</td>
                                <td> @if (@$value->all_patient_details->year != 0)
                                ...
                                <td>{{ @$value->doctor_details->first_name }}
                                    {{ @$value->doctor_details->last_name }}</td>
