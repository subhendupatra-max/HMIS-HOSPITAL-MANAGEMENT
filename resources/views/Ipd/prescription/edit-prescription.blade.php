...
<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('update-prescription-in-ipd') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <input type="hidden" name="ipd_id" value="{{ $ipdPrescriptionDetails->ipd_id }}" />
                    <input type="hidden" name="section" value="{{ $ipdPrescriptionDetails->section }}" />
                    <input type="hidden" name="patient_id" value="{{ $ipdPrescriptionDetails->patient_id }}" />
                    <input type="hidden" name="case_id" value="{{ $ipdPrescriptionDetails->case_id }}" />

                    <input type="hidden" name="id" value="{{ $ipdPrescriptionDetails->id }}" />
                    ...
                    <div class="table-responsive">
                        <table class="table card-table table-vcenter text-nowrap">
                            ...
                            <tbody id="chargeTable">
                                @if($ipdPrescriptionMedicineDetails[0]->medicine_category_id != null)
                                @foreach ($ipdPrescriptionMedicineDetails as $key=>$value)
                                <script>
                                    getMedicine_name({
                                        {
                                    ...
                                    <div class="table-responsive">
                                        <table class="table card-table table-vcenter text-nowrap">
                                            ...
                                            <tbody id="addPathologyTable">
                                                @if($ipdPrescriptionPathologyDetails[0]->test_id != null)
                                                @foreach ($ipdPrescriptionPathologyDetails as $key=>$value)
                                    ...
                                    <div class="table-responsive">
                                        <table class="table card-table table-vcenter text-nowrap">
                                            ...
                                            <tbody id="addRadiologyTable">
                                                @if($ipdPrescriptionRadiologyDetails[0]->test_id != null)
                                                @foreach ($ipdPrescriptionRadiologyDetails as $key=>$value)
                    ...
                    <div class="row  mt-5 col-md-12">
                        <div class="form-group col-md-6">
                            ...
                            <input type="datetime-local" class="form-control" id="date" name="date" required @if(isset($ipdPrescriptionDetails->prescription_date)) value="{{ date('Y-m-d h:m:s', strtotime($ipdPrescriptionDetails->prescription_date))}}" @endif>
                            @error('date')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input name="note" id="note" class="form-control" value="{{$ipdPrescriptionDetails->note}}" />
                            @error('note')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
