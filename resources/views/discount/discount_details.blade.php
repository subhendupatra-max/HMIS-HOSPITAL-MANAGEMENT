                ...
                <div class="col-lg-4 col-xl-4 border-right">
                    <div class="mt-2">
                        <div class="col-md-12 mt-3 mb-3">
                            <span class="requisition_header">Patient details : </span><span class="requisition_text">{{
                                $discount->patient_details->prefix . ' ' . $discount->patient_details->first_name . ' '
                                . $discount->patient_details->middle_name . ' ' . $discount->patient_details->last_name
                                }}<br>{{ $discount->patient_details->patient_prefix . ' ' .
                                $discount->patient_details->id }}</span>
                        </div>
                        <div class="col-md-12 mt-3 mb-3">
                            <span class="requisition_header">Section : </span>
                            @if ($discount->section == 'OPD')
                            <span class="badge badge-success">{{ $discount->section }}</span>
                            @elseif ($discount->section == 'IPD')
                            <span class="badge badge-danger">{{ $discount->section }}</span>
                            @elseif ($discount->section == 'EMG')
                            <span class="badge badge-info">{{ $discount->section }}</span>
                            @else
                            <span class="badge badge-primary">{{ $discount->section }}</span>
                            @endif
                        </div>
                        <div class="col-md-12 mt-3 mb-3">
                            <span class="requisition_header">Discount Status : </span>
                            @if ($discount->discount_status == 'Pending')
                            <span class="badge badge-warning">{{ $discount->discount_status }}</span>
                            @elseif ($discount->discount_status == 'Approved')
                            <span class="badge badge-success">{{ $discount->discount_status }}</span>
                            @else
                            <span class="badge badge-danger">{{ $discount->discount_status }}</span>
                            @endif
                        </div>
                        <div class="col-md-12 mt-3 mb-3">
                            <span class="requisition_header">Discount Requested By : </span><span
                                class="requisition_text"><span class="requisition_text">{{
                                    $discount->request_by_details->first_name }}
                                    {{ $discount->request_by_details->last_name }}</span>
                        </div>
                        <div class="col-md-12 mt-3 mb-3">
                            <span class="requisition_header">Discount Request Date : </span><span
                                class="requisition_text"><span class="requisition_text">{{ date('d-m-Y h:i A',
                                    strtotime($discount->asking_discount_time)) }}</span>
                        </div>
                        @if ($discount->discount_status == 'Approved')
                        <div class="col-md-12 mt-3 mb-3">
                        ...
                        @endif
                        @if ($discount->discount_status == 'Rejected')
                        <div class="col-md-12 mt-3 mb-3">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped card-table table-vcenter text-nowrap">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($discount_details as $bill_deta)
                                        <?php $t_bill = $t_bill + $bill_deta->bill_amount; ?>
                                        <tr>
                                            <th scope="row">{{ $loop->iteration }}</th>
                                            <td>{{ @$bill_deta->bill->bill_prefix }}{{ @$bill_deta->bill->id }}
                                            </td>
                                            <td>{{ date('d-m-Y h:i A', strtotime($bill_deta->bill->bill_date)) }}
                                            </td>
                                            <td>{{ @$bill_deta->bill_amount }}</td>
                                        </tr>
                                        @endforeach
                                        @endif
                                    </tbody>
...
<div class="modal" id="modaldemo1">
    <div class="modal-dialog" role="document">
        <div class="modal-content modal-content-demo">
            ...
            <form method="post" action="{{route('given-discount')}}">
                @csrf
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="given_discount_status" required
                                name="given_discount_status" onchange="getTotal()">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" id="total" readonly value="{{ $t_bill }}" name="total"
                                class="form-control" />
                        </div>
