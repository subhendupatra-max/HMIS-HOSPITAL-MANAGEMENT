...
<div class="col-md-12">
    <div class="card">
        ...
        <div class="options px-5 pt-5  border-bottom pb-3">
            <form method="post" action="{{route('add-radiology-billing-for-a-patient')}}">
                @csrf
                <div class="row">
                    <div class="col-md-12 mb-2">
                        <select class="form-control  select2-show-search" name="patient_id">
                            ...
                            <option value="{{@$patient->id}}" {{ @$patient_details_information->id ==
                                ...
                                {{@$patient->last_name}} ( {{@$patient->id}} )( {{@$patient->phone}} )
                            </option>
                            @endforeach
                            @endif
                        </select>
                    ...
                    <table class="table">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="py-2 px-5"><span style="color:blue">{{@$p_type }}</span></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="col-lg-7 col-xl-8">
        <form method="post" action="{{route('save-radiology-billing')}}">
            @csrf
...
<script type="text/javascript">
    ...
    function addnewrow() {
        var html = '<tr id="rowid' + i + '"><td><select required  class="form-control select2-show-search" name="test_id[]" id="test_id' + i + '" onchange="getTestAmount(this.value,' + i + ')"><option value="">Select Test Name</option> @if(isset($radiology_all_test)) @foreach ($radiology_all_test as $key => $value)<option value="{{$value->id}}">{{$value->test_name}}</option> @endforeach @endif</select></td><td><input type="text" required name="charge[]" class="form-control" onkeyup="gettotal(' + i + ')" id="charge' + i + '"></td><td><button class="btn btn-danger btn-sm" onclick="remove(' + i + ')"><i class="fa fa-times"></i></button></td></tr>';
    ...
    function getTestAmount(test_id, i) {
        ...
        // alert(patient_type);
        $.ajax({
            url: "{{ route('find-test-amount-by-test-add') }}",
            type: "POST",
            data: {
                _token: '{{ csrf_token() }}',
                testId: test_id,
