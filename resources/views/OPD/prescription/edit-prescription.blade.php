...
<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('update-prescription-in-opd') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <input type="hidden" name="opd_id" value="{{ $opdPrescriptionDetails->opd_id }}" />
                    <input type="hidden" name="section" value="{{ $opdPrescriptionDetails->section }}" />
                    <input type="hidden" name="patient_id" value="{{ $opdPrescriptionDetails->patient_id }}" />
                    <input type="hidden" name="case_id" value="{{ $opdPrescriptionDetails->case_id }}" />

                    <input type="hidden" name="id" value="{{ $opdPrescriptionDetails->id }}" />
                    ...
                    <div class="table-responsive">
                        <table class="table card-table table-vcenter text-nowrap">
                            ...
                            <tbody id="chargeTable">
                                @if($opdPrescriptionMedicineDetails[0]->medicine_category_id != null)
                                @foreach ($opdPrescriptionMedicineDetails as $key=>$value)
                                <script>
                                    getMedicine_name({
                                        {
                                    ...
                                    <div class="table-responsive">
                                        <table class="table card-table table-vcenter text-nowrap">
                                            ...
                                            <tbody id="addPathologyTable">
                                                @if($opdPrescriptionPathologyDetails[0]->test_id != null)
                                    ...
                                    <div class="table-responsive">
                                        <table class="table card-table table-vcenter text-nowrap">
                                            ...
                                            <tbody id="addRadiologyTable">
                                                @if($opdPrescriptionRadiologyDetails[0]->test_id != null)
                    ...
                    <div class="row  mt-5 col-md-12">
                        <div class="form-group col-md-6">
                            ...
                            <input type="datetime-local" class="form-control" id="date" name="date" required @if(isset($opdPrescriptionDetails->prescription_date)) value="{{ date('Y-m-d h:m:s', strtotime($opdPrescriptionDetails->prescription_date))}}" @endif>
                            @error('date')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input name="note" id="note" class="form-control" value="{{$opdPrescriptionDetails->note}}" />
                            @error('note')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
