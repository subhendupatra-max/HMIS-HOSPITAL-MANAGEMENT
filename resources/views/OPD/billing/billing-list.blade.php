            ...
            <div class="card-header d-block">
                <div class="row">
                    ...
                    <div class="col-md-8 text-right">
                        <div class="d-block">
                            @can('add opd billing')
                                <a href="{{ route('add-opd-billing', ['id' => base64_encode($opd_id)]) }}"
                                    ...
                                    @foreach ($opd_billing_details as $value)
                                        <tr>
                                            <td><a class="text-info"
                                                    href="{{ route('opd-bill-details', ['bill_id' => base64_encode($value->id)]) }}">{{ $value->bill_prefix }}{{ $value->id }}</a>
                                            </td>
                                            <td>{{ date('d-m-Y h:i A', strtotime($value->bill_date)) }}</td>
                                            <td>{{ $value->total_amount }}</td>
                                            <td>{{ $value->created_details->first_name }}
                                                {{ $value->created_details->last_name }}</td>
